...
이 설정은 기존 디자인 PDF를 그대로 복제하는 방식이 아니라,
Apps Script가 학생별 결과 요약 PDF를 새로 만들어 Google Drive에 저장하는 방식입니다.
...
- 시트에서 `resultPdfUrl`을 클릭하면 선생님이 결과 PDF를 바로 열 수 있습니다.
- 같은 `attemptId`로 다시 제출되면 기존 행을 업데이트하고, 기존 PDF는 휴지통으로 보낸 뒤 새 PDF를 만듭니다.
- 더 예쁜 브랜딩 PDF나 기존 결과지와 동일한 다페이지 PDF가 필요하면, 그건 별도 템플릿 기반 생성 작업으로 확장해야 합니다.
